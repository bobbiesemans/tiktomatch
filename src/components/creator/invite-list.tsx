...
import { ContractModal } from "@/components/contracts/accept-modal"
import { formatEuro, scoreColor } from "@/lib/utils"
import { CheckCircle, Clock, Gift, MessageSquare } from "lucide-react"
...
export function InviteList({ invites }: Props) {
  ...
  const contractInvite = invites.find((i) => i.id === contractMatchId)

  function handleAcceptWithContract(matchId: string) {
    setContractMatchId(null)
    startTransition(() => acceptMatch(matchId))
  }
  ...
  return (
    <>
      {/* Contract modal */}
      {contractMatchId && contractInvite && (
        <ContractModal
          matchId={contractMatchId}
          brandName={contractInvite.brands?.bedrijfsnaam ?? "Brand"}
          campagneType={contractInvite.campagne_type}
          onAccepted={handleAcceptWithContract}
          onCancelled={() => setContractMatchId(null)}
        />
      )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {invite.campagne_type && (
                      <Badge variant="secondary" className="text-xs capitalize">{invite.campagne_type}</Badge>
                    )}
                  ...
                  <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        asChild
                      >
                        <a href={`/dashboard/creator/berichten?match=${invite.id}`}>
                      ...
                      <Button
                        ...
                        onClick={() => setContractMatchId(invite.id)}
                      >
                        Accepteren
                      </Button>
                      <Button
